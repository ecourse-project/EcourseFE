	...
	reducers: {
		...
		updateDocIsFav: (state, action: PayloadAction<Document>) => {
			if (action.payload.is_favorite) {
				...
				const favIdx = state.listFav.documents?.findIndex(
					(v) => v.id === action.payload.id
				);
				if (favIdx >= 0) {
					state.listFav.documents = state.listFav.documents.filter(
						(v) => v.id !== action.payload.id
					);
				}
			}
		...
		updateCourseIsFav: (state, action: PayloadAction<Course>) => {
			if (action.payload.is_favorite) {
				...
				const favIdx = state.listFav.courses?.findIndex(
					(v) => v.id === action.payload.id
				);
				if (favIdx >= 0) {
					state.listFav.courses = state.listFav.courses.filter(
						(v) => v.id !== action.payload.id
					);
				}
			}
